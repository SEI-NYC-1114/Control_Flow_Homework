...
function prompt9() {
  // YOUR CODE HERE
  for (i = starWars.length; i <= starWars.length; ) {
...
// 🌟 MAKE A COMMIT: "Complete prompt 9" //NOT WORKING 
...
//declaring a new array in numerical order
let sortedNums = [];

function prompt11() {
  let median;
  for (i = 0; i <= nums.length; i++) {
    //iterates through nums but puts each number into position in sorted nums of same value
    sortedNums.splice(nums[i], 0, nums[i]);
  };
...
function prompt13() {
  ...
  for (i = 0; i <= nestedArrays.length; i++) {
    for (j = 0; j <= nextedArrays[i].length; j++) {
